...
'Thumbs.db', 'ehthumbs.db', 'ehthumbs_vista.db'].map(x => x.toLowerCase()));
...
'.nfs',
// macOS resource forks
'._'].map(x => x.toLowerCase());

function isIgnored(node) {
  let { name } = node.path;
  let lower = name.toLowerCase();
  let ignore = IgnoreNames.has(lower) || IgnorePrefixes.some(p => lower.substr(0, p.length) === p);
  // if (ignore) {
